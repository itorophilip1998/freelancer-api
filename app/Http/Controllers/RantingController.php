...
class RantingController extends Controller
{
    public function add()
    {
        try {
            ...
            if ($rater_exist) {
                ...
                // ]);
                return response()->json(['message' => 'Rating Already Exists 👍', 'ranting' => $rater_exist], 200);
            }
    ...
    public function get($user_id)
    {
        try {
            ...
            return response()->json(['message' => 'Rating successfully Loaded 👍', 'rate_star' => $newrate, 'ratings' => $ranting], 200);
        } catch (\Throwable $th) {
            //   throw $th;
            return response()->json([
                'message' => 'This error is from the backend, please contact the backend developer'
